# 1 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino"
# 2 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 3 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 4 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 5 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 6 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 7 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 8 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 9 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 10 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 11 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 12 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 13 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 14 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 15 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2

# 17 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 18 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 19 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 20 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 21 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 22 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
# 23 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 2
...
static constexpr int MAP_PLOT_W = 240 - 16;
...
static esp_err_t wifiScanStartResult = 0 /*!< esp_err_t value indicating success (no error) */;
static volatile bool wifiScanTaskRunning = false;
static volatile bool wifiScanTaskDone = false;
static volatile int16_t wifiScanTaskStatus = (-2);
...
static lv_color_t lvBuf1[240 * 24];
static lv_color_t lvBuf2[240 * 24];
...
static long cachedMapTileX = 
# 320 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 3 4
                            (-0x7fffffffL - 1L)
# 320 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino"
                                    ;
static long cachedMapTileY = 
# 321 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 3 4
                            (-0x7fffffffL - 1L)
# 321 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino"
                                    ;
static int cachedMapPixelX = 
# 322 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 3 4
                            (-0x7fffffff - 1)
# 322 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino"
                                   ;
static int cachedMapPixelY = 
# 323 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino" 3 4
                            (-0x7fffffff - 1)
# 323 "C:\\Users\\justi\\Documents\\Arduino\\s3-lora-interface\\s3-lora-interface.ino"
                                   ;
...
static void initSdStorage() {
  HWCDCSerial.printf("[sd] setPins clk=%d cmd=%d d0=%d d1=%d d2=%d d3=%d oneBit=%s freq=%d\n",
                38, 40, 39, 41, 48, 47,
                false ? "true" : "false", 20000);
  SD_MMC.end();
  bool pinsOk = false
  ...
  if (!pinsOk) {
    ...
    HWCDCSerial.println("[sd] setPins failed");
    return;
  }
  ...
  if (!SD_MMC.begin("/sdcard", false, false, 20000)) {
    ...
    HWCDCSerial.println("[sd] mount failed");
    return;
  }
  ...
  if (cardType == CARD_NONE) {
    ...
    HWCDCSerial.println("[sd] card type none");
    SD_MMC.end();
    return;
  }
  ...
  HWCDCSerial.printf("[sd] mounted type=%s card=%llu total=%llu used=%llu\n",
                sdStorage.cardType,
...
static void saveLastLocationToSd(double lat, double lon, int32_t alt) {
  ...
  File file = SD_MMC.open(SD_LAST_LOCATION_PATH, "w");
  if (!file) return;
...
static bool loadLastLocationFromSd() {
  ...
  File file = SD_MMC.open(SD_LAST_LOCATION_PATH, "r");
...
static int batteryPercentFromMv(uint32_t mv) {
  ...
  for (size_t i = 1; i < sizeof(curve) / sizeof(curve[0]); ++i) {
    if (mv <= curve[i].mv) {
      return ((interpolateBatteryPct(mv, curve[i - 1].mv, curve[i - 1].pct, curve[i].mv, curve[i].pct)) < (0) ? (0) : ((interpolateBatteryPct(mv, curve[i - 1].mv, curve[i - 1].pct, curve[i].mv, curve[i].pct)) > (100) ? (100) : (interpolateBatteryPct(mv, curve[i - 1].mv, curve[i - 1].pct, curve[i].mv, curve[i].pct))));
    }
  }
...
static uint32_t smoothBatteryEstimate(uint32_t currentMv, uint32_t targetMv, bool pluggedIn) {
  if (currentMv == 0) return ((targetMv) < (3300UL) ? (3300UL) : ((targetMv) > (4200UL) ? (4200UL) : (targetMv)));
  targetMv = ((targetMv) < (3300UL) ? (3300UL) : ((targetMv) > (4200UL) ? (4200UL) : (targetMv)));
...
static void sampleLocalBattery() {
  ...
  uint32_t adcMv = analogReadMilliVolts(9);
  uint32_t packMv = (uint32_t)(adcMv * 2.0f + 0.5f);
...
static void startWifiAp() {
  ...
  bool ok = WiFi.softAP("Heltec-LoRa-Interface", "12345678");
  if (ok) {
    server.begin();
    wifiEnabled = true;
...
static void renderIdfWifiScanResults(uint16_t count) {
  ...
  if (err != 0 /*!< esp_err_t value indicating success (no error) */) {
    ...
    HWCDCSerial.printf("[wifi] scan read failed err=%d\n", (int)err);
    return;
  }
...
static void renderStoredWifiScanResults(int16_t status) {
  if (!listWifiScan) return;
  lv_obj_clean(listWifiScan);
  if (status == (-2) || status <= 0 || wifiScanResultCount == 0) {
    if (lblWifiScanStatus) lv_label_set_text(lblWifiScanStatus, status == 0 ? "No networks found" : "Scan failed");
    return;
  }
...
static void wifiScanTask(void*) {
  ...
  HWCDCSerial.println("[wifi] task scan starting");
  WiFi.mode(WIFI_MODE_STA);
  WiFi.scanDelete();
  ...
  HWCDCSerial.printf("[wifi] task scan complete status=%d count=%u\n", status, (unsigned)resultCount);
  vTaskDelete(nullptr);
}
...
static void startWifiScan() {
  ...
  HWCDCSerial.printf("[wifi] scan task create=%ld\n", (long)ok);
  if (ok != ( ( ( BaseType_t ) 1 ) )) {
    wifiScanTaskRunning = false;
    wifiScanActive = false;
...
static void pollWifiScan() {
  ...
  if (millis() - wifiScanStartedMs > 9000) {
    wifiScanActive = false;
    HWCDCSerial.println("[wifi] task scan timeout");
    if (lblWifiScanStatus) lv_label_set_text(lblWifiScanStatus, "Scan timed out");
  }
}

static void applyBacklight() {
  uint8_t pct = ((backlightPercent) < (10) ? (10) : ((backlightPercent) > (100) ? (100) : (backlightPercent)));
...
static void initBacklight() {
  pinMode(45, 0x03);
  backlightPwmReady = ledcAttach(45, 5000, 8);
  applyBacklight();
}

static void backlightSliderEvent(lv_event_t* e) {
  lv_obj_t* slider = (lv_obj_t*)lv_event_get_target(e);
  backlightPercent = (((int)lv_slider_get_value(slider)) < (10) ? (10) : (((int)lv_slider_get_value(slider)) > (100) ? (100) : ((int)lv_slider_get_value(slider))));
  applyBacklight();
}
...
static bool readTouch(uint16_t& x, uint16_t& y) {
  ...
  if (Wire.requestFrom(0x38, (uint8_t)5) != 5) return false;
  ...
  uint16_t rawX = ((((uint16_t)(xh & 0x0F) << 8) | xl) < (0) ? (0) : ((((uint16_t)(xh & 0x0F) << 8) | xl) > (240 - 1) ? (240 - 1) : (((uint16_t)(xh & 0x0F) << 8) | xl)));
  uint16_t rawY = ((((uint16_t)(yh & 0x0F) << 8) | yl) < (0) ? (0) : ((((uint16_t)(yh & 0x0F) << 8) | yl) > (320 - 1) ? (320 - 1) : (((uint16_t)(yh & 0x0F) << 8) | yl)));
...
static lv_obj_t* makePage(lv_obj_t* parent) {
  ...
  lv_obj_set_size(page, 240, 320 - NAV_BAR_H - STATUS_BAR_H);
...
static void buildStatusBar(lv_obj_t* screen) {
  ...
  lv_obj_set_size(statusBar, 240, STATUS_BAR_H);
  ...
  lv_obj_set_style_text_font(lblBatteryStatus, &lv_font_montserrat_14, 0);
...
static void buildNavBar(lv_obj_t* screen) {
  ...
  lv_obj_set_size(navBar, 240, NAV_BAR_H);
...
static void setUiLandscape(bool landscape) {
  ...
  dispDrv.hor_res = landscape ? 320 : 240;
  dispDrv.ver_res = landscape ? 240 : 320;
...
static void ensureWifiScanPage() {
  ...
  lv_obj_set_size(listWifiScan, 240 - 12, 180);
...
static void buildLandscapeKeyboardScreen() {
  ...
  lv_obj_set_size(keyboardText, 320 - 16, 50);
  ...
  lv_obj_set_size(landscapeKeyboard, 320, 156);
...
static void buildScreenUi() {
  ...
  lv_obj_set_style_text_font(launcherTitle, &lv_font_montserrat_14, 0);
  ...
  lv_obj_set_size(loraPanel, 240 - 12, 112);
  ...
  lv_obj_set_size(taPublicInput, 240 - 76, 38);
  ...
  lv_obj_set_size(taPrivateInput, 240 - 76, 38);
  ...
  for (size_t i = 0; i < MAP_DOT_COUNT; i++) {
    ...
    lv_obj_set_style_radius(mapDots[i], 0x7FFF /**< A very big radius to always draw as circle*/, 0);
  ...
  lv_obj_set_width(lblMapStats, 240 - 16);
  ...
  lv_obj_set_size(interfacePanel, 240 - 12, 226);
  ...
  lv_obj_set_size(serialPanel, 240 - 12, 226);
  ...
  lv_obj_set_size(radioPanel, 240 - 12, 226);
  ...
  lv_obj_set_size(gpsPanel, 240 - 12, 226);
  ...
  lv_obj_set_size(wifiPanel, 240 - 12, 150);
  ...
  lv_obj_set_size(wifiStatsPanel, 240 - 12, 226);
  ...
  lv_obj_set_size(backlightPanel, 240 - 12, 150);
  ...
  lv_obj_set_size(sliderBacklight, 240 - 42, 24);
  ...
  lv_obj_set_size(batteryPanel, 240 - 12, 226);
  ...
  lv_obj_set_size(keyboard, 240, 132);
...
static void initScreen() {
  ...
  lv_init();
  lv_disp_draw_buf_init(&drawBuf, lvBuf1, lvBuf2, 240 * 24);
...
static double distanceMeters(double lat1, double lon1, double lat2, double lon2) {
  ...
  double p1 = lat1 * 0.017453292519943295769236907684886;
  double p2 = lat2 * 0.017453292519943295769236907684886;
  double dp = (lat2 - lat1) * 0.017453292519943295769236907684886;
  double dl = (lon2 - lon1) * 0.017453292519943295769236907684886;
...
static long latToTileY(double lat, int zoom) {
  lat = ((lat) < (-85.05112878) ? (-85.05112878) : ((lat) > (85.05112878) ? (85.05112878) : (lat)));
  double latRad = lat * 0.017453292519943295769236907684886;
  double n = (double)(1UL << zoom);
  return (long)floor((1.0 - log(tan(latRad) + 1.0 / cos(latRad)) / 3.1415926535897932384626433832795) / 2.0 * n);
}
...
static double latToGlobalPixelY(double lat, int zoom) {
  ...
  double latRad = lat * 0.017453292519943295769236907684886;
  double n = (double)(1UL << zoom) * MAP_TILE_SIZE;
  return (1.0 - log(tan(latRad) + 1.0 / cos(latRad)) / 3.1415926535897932384626433832795) / 2.0 * n;
}
...
static void loadMapCacheFromSd() {
  ...
  if (!SD_MMC.exists(SD_MAP_CACHE_PATH)) {
    ...
    HWCDCSerial.printf("[MAPCACHE] %s path=%s\n", mapCacheStatus, SD_MAP_CACHE_PATH);
    return;
  }
  File file = SD_MMC.open(SD_MAP_CACHE_PATH, "r");
  if (!file) {
  ...
  if (header.magic != MAP_CACHE_MAGIC ||
    ...
    HWCDCSerial.printf("[MAPCACHE] %s magic=%08lX ver=%u size=%ux%u expected=%ux%u\n",
                  mapCacheStatus,
  ...
  HWCDCSerial.printf("[MAPCACHE] loaded %.6f,%.6f z%d tile=%ld/%ld found=%s bytes=%u\n",
                cachedMapLat,
                cachedMapLon,
                cachedMapZoom,
                cachedMapTileX,
...
static void saveMapCacheToSd() {
  ...
  File file = SD_MMC.open(SD_MAP_CACHE_PATH, "w");
  if (!file) {
  ...
  HWCDCSerial.printf("[MAPCACHE] %s %.6f,%.6f z%d tile=%ld/%ld header=%u image=%u\n",
                mapCacheStatus,
                cachedMapLat,
                cachedMapLon,
                cachedMapZoom,
...
static void refreshScreenUi() {
  ...
  if (lblWifiState) {
    ...
    snprintf(wifiStateText, sizeof(wifiStateText),
             "Status: %s\nSSID: %s\nIP: %s",
             wifiState,
             wifiEnabled ? (wifiApMode ? "Heltec-LoRa-Interface" : wifiLocalSsid) : "-",
             wifiIp.c_str());
...
static bool sendConfigRequest() {
  meshtastic_ToRadio toRadio = {0, {{0, 0, 0, 0, {{meshtastic_PortNum_UNKNOWN_APP, {0, {0}}, 0, 0, 0, 0, 0, 0, false, 0}}, 0, 0, 0, 0, 0, meshtastic_MeshPacket_Priority_UNSET, 0, meshtastic_MeshPacket_Delayed_NO_DELAY, 0, 0, {0, {0}}, 0, 0, 0, 0, meshtastic_MeshPacket_TransportMechanism_TRANSPORT_INTERNAL}}};
  toRadio.which_payload_variant = 3;
  toRadio.want_config_id = 1;

  uint8_t out[64];
  pb_ostream_t stream = pb_ostream_from_buffer(out, sizeof(out));
  if (!pb_encode(&stream, &meshtastic_ToRadio_msg, &toRadio)) return false;
...
static bool sendLocalAdmin(const meshtastic_AdminMessage& admin) {
  ...
  if (!pb_encode(&adminStream, &meshtastic_AdminMessage_msg, &admin)) return false;
...
static bool sendHeltecReboot(uint8_t seconds) {
  meshtastic_AdminMessage admin = {0, {0}, {0, {0}}};
  admin.which_payload_variant = 97;
...
static bool sendHeltecLoraConfig(const String& region, const String& preset, uint8_t hopLimit, int8_t txPower) {
  meshtastic_AdminMessage admin = {0, {0}, {0, {0}}};
  admin.which_payload_variant = 34;
  admin.set_config.which_payload_variant = 6;
...
static bool sendHeltecSerialConfig(bool enabled, uint32_t rxd, uint32_t txd, const String& baud, const String& mode, bool echo, bool overrideConsole) {
  meshtastic_AdminMessage admin = {0, {0}, {0, {0}}};
  admin.which_payload_variant = 35;
  admin.set_module_config.which_payload_variant = 2;
...
static bool sendHeltecOwnerName(const String& name, const String& shortName) {
  ...
  meshtastic_AdminMessage admin = {0, {0}, {0, {0}}};
  admin.which_payload_variant = 32;
...
static bool sendHeltecDeviceConfig(const String& role, const String& rebroadcast, uint32_t nodeInfoSecs, const String& tz, bool ledOff, const String& buzzer) {
  meshtastic_AdminMessage admin = {0, {0}, {0, {0}}};
  admin.which_payload_variant = 34;
  admin.set_config.which_payload_variant = 1;
...
static bool sendHeltecPositionConfig(bool gpsEnabled, const String& gpsMode, bool fixedPosition, bool smartBroadcast, uint32_t broadcastSecs, uint32_t gpsUpdateSecs, uint32_t gpsAttemptSecs, uint32_t smartMinMeters, uint32_t smartMinSecs) {
  meshtastic_AdminMessage admin = {0, {0}, {0, {0}}};
  admin.which_payload_variant = 34;
  admin.set_config.which_payload_variant = 2;
...
static bool sendHeltecPowerConfig(bool powerSaving, uint32_t shutdownSecs, uint32_t waitBluetoothSecs, uint32_t sdsSecs, uint32_t lsSecs, uint32_t minWakeSecs) {
  meshtastic_AdminMessage admin = {0, {0}, {0, {0}}};
  admin.which_payload_variant = 34;
  admin.set_config.which_payload_variant = 3;
...
static bool sendHeltecCommit() {
  meshtastic_AdminMessage admin = {0, {0}, {0, {0}}};
  admin.which_payload_variant = 65;
...
static bool sendHeltecChannelConfig(uint8_t index, const String& role, const String& name, const String& psk, bool uplink, bool downlink) {
  meshtastic_AdminMessage admin = {0, {0}, {0, {0}}};
  admin.which_payload_variant = 33;
...
static void updateTelemetry(uint32_t from, const meshtastic_Data& data) {
  meshtastic_Telemetry telemetry = {0, 0, {{false, 0, false, 0, false, 0, false, 0, false, 0}}};
  pb_istream_t stream = pb_istream_from_buffer(data.payload.bytes, data.payload.size);
  if (!pb_decode(&stream, &meshtastic_Telemetry_msg, &telemetry)) return;

  char line[160];
  if (telemetry.which_variant == 2) {
    ...
    appendLine(eventLog, LOG_SIZE, line);
  } else if (telemetry.which_variant == 6) {
    lastTelemetryMs = millis();
...
static void handleDecodedPacket(const meshtastic_MeshPacket& packet) {
  ...
  if (packet.which_payload_variant == 5) {
    encryptedPackets++;
    char line[96];
  ...
  if (packet.which_payload_variant != 4) return;
  ...
  if (data.portnum == meshtastic_PortNum_TEXT_MESSAGE_APP) {
    ...
    positionPackets++;
    lastPortNum = data.portnum;
    meshtastic_Position position = {false, 0, false, 0, false, 0, 0, meshtastic_Position_LocSource_LOC_UNSET, meshtastic_Position_AltSource_ALT_UNSET, 0, 0, false, 0, false, 0, 0, 0, 0, 0, false, 0, false, 0, 0, 0, 0, 0, 0, 0, 0};
    pb_istream_t stream = pb_istream_from_buffer(data.payload.bytes, data.payload.size);
    if (pb_decode(&stream, &meshtastic_Position_msg, &position) &&
    ...
    nodeInfoPackets++;
    lastPortNum = data.portnum;
    meshtastic_User user = {"", "", "", {0}, meshtastic_HardwareModel_UNSET, 0, meshtastic_Config_DeviceConfig_Role_CLIENT, {0, {0}}, false, 0};
    pb_istream_t stream = pb_istream_from_buffer(data.payload.bytes, data.payload.size);
    if (pb_decode(&stream, &meshtastic_User_msg, &user)) {
...
static void decodeFromRadio(const uint8_t* payload, size_t len) {
  meshtastic_FromRadio fromRadio = {0, 0, {{0, 0, 0, 0, {{meshtastic_PortNum_UNKNOWN_APP, {0, {0}}, 0, 0, 0, 0, 0, 0, false, 0}}, 0, 0, 0, 0, 0, meshtastic_MeshPacket_Priority_UNSET, 0, meshtastic_MeshPacket_Delayed_NO_DELAY, 0, 0, {0, {0}}, 0, 0, 0, 0, meshtastic_MeshPacket_TransportMechanism_TRANSPORT_INTERNAL}}};
  pb_istream_t stream = pb_istream_from_buffer(payload, len);
  if (!pb_decode(&stream, &meshtastic_FromRadio_msg, &fromRadio)) {
    decodeErrors++;
  ...
  } else if (fromRadio.which_payload_variant == 6) {
    char line[420];
    snprintf(line, sizeof(line), "[node log] %.380s\n", fromRadio.log_record.message);
    appendLine(eventLog, LOG_SIZE, line);
  } else if (fromRadio.which_payload_variant == 10) {
    configFrames++;
    updateChannelRecord(fromRadio.channel);
  } else if (fromRadio.which_payload_variant == 5 ||
             fromRadio.which_payload_variant == 9 ||
             fromRadio.which_payload_variant == 7) {
    configFrames++;
  } else {
    otherFrames++;
  }
...
static String buildStatusJson() {
  ...
  json += "\"myNode\":\"!" + String(stats.myNodeNum, 16) + "\",";
  ...
  for (size_t i = 0; i < nodeCount; i++) {
    if (i) json += ",";
    json += "{\"num\":\"!" + String(nodes[i].num, 16) + "\",";
...
static void handleStatusSnapshot() {
  ...
  File file = SD_MMC.open(SD_STATUS_SNAPSHOT_PATH, "w");
  if (!file) {
    sdStorage.writeErrors++;
...
void setup() {
  ...
  HWCDCSerial.println("[boot] serial ready");
  SerialLoRa.setRxBufferSize(4096);
  SerialLoRa.begin(115200, SERIAL_8N1, 14, 21);
